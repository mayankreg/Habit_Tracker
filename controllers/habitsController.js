...
module.exports.weeklyview = (req, res) => {
  ...
  for (let i = 0; i < 7; i++) {
    let d = date.getDate() + ',' + Month[date.getMonth()] + ' ' + date.getFullYear();
      // console.log('date to be pushed in array', d);
    date.setDate(date.getDate() - 1); //decrease date one by one
      // console.log('reduced date', date.getDate());
    days.push(d);
  }
  ...
  Habit.find({}, function (error, habits) {
    // console.log('before habit', habits);
    updateData(habits);
    // console.log('after habits', habits);
    if (error) {
...
module.exports.update = (req, res) => {
  ...
  // console.log('this is :', id, day, status);
...
let updateData = (habits) => {
  ...
  console.log('in updatedata...', todaysDate);
  ...
  for (let habit of habits) {
    ...
    console.log('for habit:', habit, 'difference:', diff);
